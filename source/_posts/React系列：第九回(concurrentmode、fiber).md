...
render做的事情很清晰，就是**根据element的信息，生成真实的dom，更新props，递归子节点，最后完成挂载**。现在设想一下，如果我们传入的是一个巨深的虚拟dom，那么会发生什么？render这个函数的耗时势必也会剧增,`一旦开始执行，就会执行到底`。即：从首次执行render开始到最终挂载结束，期间的浏览器都是处于阻塞的状态。此时是无法响应任何用户的操作的，俗称`卡死`。
...
总体的逻辑就是：**深度递归的规则，一个节点一个节点的往深处走，然后不停会退直至扫完所有节点。在扫的过程中，创建fiber，创建dom，挂载dom**
注意：
